...
function pingColor(bars) {
  if (bars >= 4) return '#55FF55'; // verde clásico de Minecraft
  if (bars >= 2) return '#FFAA00'; // amarillo/naranja
  return '#FF5555'; // rojo
}

function SignalBars({ ping, loading }) {
  if (loading) return <span className="mc-server-signal mc-server-signal-loading" title="…" />;
  const bars = pingBars(ping);
  const color = ping == null ? '#7a7a7a' : pingColor(bars);
  return (
    <div className="mc-server-signal" title={ping != null ? `${ping} ms` : ''}>
      {[1, 2, 3, 4, 5].map((i) => (
        <span
          key={i}
...
export default function MinecraftServerList() {
  ...
  // mismo criterio que usa nbtWriter.js al armar el servers.dat.
