...
function UpdateFooter() {
  ...
  // new Date().getFullYear() en vez de un año fijo: así el aviso de
  // derechos de autor sigue siendo correcto solo, sin que haya que tocar
  // este archivo a mano cada enero mientras la app siga viva.
  ...
  return (
    <div className="settings-footer">
      ...
      <div className="settings-footer-copyright">{t('settings.copyright', { year })}</div>
    </div>
  );
}
...
// El punto de acento de la preview ya no viene hardcodeado en rojo: se le
// pasa el color realmente elegido (accentColor) para que las tarjetas de
// Ajustes > Apariencia siempre muestren el combo real oled/dark/light/
// system + rojo/morado que va a quedar aplicado, no un rojo fijo que
// desentonaría apenas alguien elija morado.
function ThemePreview({ id, accentHex }) {
  const palettes = {
    dark: { bg: '#222326', bar: '#3c3e46' },
    light: { bg: '#ffffff', bar: '#e8e4dc' },
    oled: { bg: '#000000', bar: '#232329' },
    system: { bg: 'linear-gradient(135deg, #222326 50%, #ffffff 50%)', bar: '#8a8b90' },
  };
  const p = palettes[id];
  return (
    <div className="theme-option-preview" style={{ background: p.bg }}>
      <div className="theme-preview-dot" style={{ background: accentHex }} />
...
// Mismos dos tonos que --accent-gradient en theme.css ([data-accent='red']
// implícito del :root y [data-accent='purple']) — se repiten acá a mano en
// vez de leerlos con getComputedStyle porque esta preview se dibuja para
// AMBAS opciones a la vez (incluida la que no está aplicada todavía al
// documento), así que no hay forma de leer la variable CSS de la que no
// está activa. Si se retocan esos degradés en theme.css, hay que retocar
// esto también.
const ACCENT_SWATCHES = {
  red: { primary: '#d81f33', gradient: 'linear-gradient(135deg, #ef4457 0%, #740f1a 100%)' },
  purple: { primary: '#7c3aff', gradient: 'linear-gradient(135deg, #9863ff 0%, #3c1499 100%)' },
};

// Preview del selector de acento: en vez de un puntito de color plano,
// muestra en miniatura los dos lugares donde ese color realmente aparece
// en el launcher — como relleno (el botón degradé, ej. "Usar este ícono")
// y como borde/resalte (tarjetas y campos seleccionados) — para que elegir
// acá se sienta igual de informado que elegir un tema, no una casilla
// suelta aparte.
function AccentPreview({ id }) {
  const p = ACCENT_SWATCHES[id];
  return (
    <div className="theme-option-preview" style={{ background: '#1a1b1f' }}>
      <div className="accent-preview-btn" style={{ background: p.gradient }} />
      <div className="accent-preview-outline" style={{ borderColor: p.primary }} />
    </div>
  );
}
...
function AppearanceSection() {
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      <h3 style={{ marginTop: 0 }}>{t('settings.accentTitle')}</h3>
      ...
      <div className="theme-grid">
        {accents.map((accent) => (
          <button
            key={accent.id}
            type="button"
            className={'theme-option' + (accentColor === accent.id ? ' selected' : '')}
            onClick={() => updateSettings({ accentColor: accent.id })}
          >
            <AccentPreview id={accent.id} />
      ...
      <div className="theme-grid">
        {themes.map((theme) => (
          ...
          >
            <ThemePreview id={theme.id} accentHex={ACCENT_SWATCHES[accentColor].primary} />
            <div className="theme-option-label">
              <span className="theme-option-radio" />
              {theme.label}
            </div>
...
function BehaviorSection() {
  ...
  return (
    <div style={{ maxWidth: 480 }}>
      ...
      >
        <div>
          <div style={{ fontWeight: 600, marginBottom: 4 }}>{t('settings.launchAtStartup')}</div>
          <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{t('settings.launchAtStartupHint')}</div>
        </div>
        <div
          ...
          <div key={acc.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 12, gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                {activeAccount?.id === acc.id && <span className="badge badge-active" style={{ marginLeft: 6 }}>{t('common.active')}</span>}
              </div>
            </div>
