 ...
 */
async function buildLaunchCommand(instance, account, installResult, javaBin, directConnect, quickPlaySingleplayer) {
  ...
  if (directConnect) {
    ...
    values.quickPlayMultiplayer = directConnect.port && directConnect.port !== 25565
      ...
      : directConnect.host;
  } else if (quickPlaySingleplayer) {
    // Botón "Jugar" de un mundo puntual (pestaña "Mundos"): mismo mecanismo
    // de Quick Play que directConnect de arriba, pero para Un Jugador
    // (${quickPlaySingleplayer}, también camelCase). El valor esperado acá
    // es el nombre de la CARPETA del mundo dentro de /saves (no el
    // "LevelName" que se ve en el menú — ver WorldCard.jsx/buildWorldEntry
    // en instanceStore.js, que es justo lo que le llega en `world.name`).
    values.quickPlaySingleplayer = quickPlaySingleplayer;
  }
  ...
  const features = {
    ...
    has_quick_plays_support: !!directConnect || !!quickPlaySingleplayer,
  ...
  // A diferencia de Quick Play Multiplayer, Un Jugador nunca tuvo un flag
  // clásico de respaldo (--server/--port es específico de multijugador) —
  // versiones anteriores a 1.20 simplemente no tienen forma de pedirle al
  // cliente que entre directo a un mundo puntual por línea de comandos, así
  // que en esos casos el juego igual arranca bien, solo que abre en el menú
  // principal en vez de entrar solo al mundo. `usedQuickPlaySingleplayer`
  // se devuelve para que launch() pueda avisar de esto en el log en vez de
  // fallar en silencio.
  const usedQuickPlaySingleplayer = fullArgs.includes('--quickPlaySingleplayer');

  return { javaBin, args: fullArgs, cwd: instance.dir, fullscreen, usedQuickPlaySingleplayer };
}
...
// Complemento de CONNECTING_TO_RE: esta es la línea que el cliente vanilla
// imprime SIEMPRE que se desconecta de un server — tanto al tocar
// "Disconnect"/"Save and quit to title" desde el menú de pausa o el chat,
// como al perder la conexión o ser kickeado. La tira ClientCommonPacketListenerImpl
// (net.minecraft.client.multiplayer) es la clase vanilla real (desde el
// refactor de packet listeners en 1.20.2) que loguea esto antes de volver
// al menú/lista de servidores; no es algo específico de ningún mod ni
// server. Antes de este fix, la Rich Presence sólo se enteraba de que el
// jugador se fue de un server cuando se unía a OTRO (nueva línea
// "Connecting to...") o cuando cerraba el juego entero (onExit) — si volvía
// al menú Multijugador sin hacer ninguna de las dos cosas, currentServer
// se quedaba pisado mostrando el server viejo para siempre. Se matchea sin
// atarse al nombre de la clase (sólo "disconnected with reason") para no
// romperse si el log trae un prefijo de logger distinto entre versiones.
const DISCONNECTED_RE = /disconnected with reason/i;

async function launch(instance, account, hooks = {}, directConnect = null, quickPlaySingleplayer = null) {
  const { onProgress, onLog, onExit, onServerConnect, onServerDisconnect } = hooks;
  ...
  if (directConnect) {
    ...
    onLog?.(`[Hard Launcher] Entrando directo al mundo "${quickPlaySingleplayer}"...`);
  }

  const { args, cwd, fullscreen, usedQuickPlaySingleplayer } = await buildLaunchCommand(
    instance,
    account,
    installResult,
    javaBin,
  ...
  if (quickPlaySingleplayer && !usedQuickPlaySingleplayer) {
    onLog?.(
      '[Hard Launcher] Esta versión de Minecraft es anterior a 1.20 y no soporta entrar directo a un mundo: se abre en el menú principal.'
    );
  }
  ...
  proc.stdout.on('data', (data) => {
    ...
    else if (DISCONNECTED_RE.test(text)) onServerDisconnect?.();
  });
